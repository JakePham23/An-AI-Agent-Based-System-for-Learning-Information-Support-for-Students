...
export default function Settings() {
  ...
  const setGraphMaxNodes = useCallback((nodes: number) => {
    const maxLimit = Math.max(backendMaxGraphNodes || 0, 20000)
      ...
      <Popover open={opened} onOpenChange={setOpened}>
        ...
        >
          <div className="flex flex-col gap-2">
            ...
            />
            <LabeledNumberInput
              label={`${t('graphPanel.sideBar.settings.maxNodes')} (≤ ${Math.max(backendMaxGraphNodes || 0, 20000)})`}
              min={1}
              max={Math.max(backendMaxGraphNodes || 0, 20000)}
              value={graphMaxNodes}
              defaultValue={backendMaxGraphNodes || 20000}
