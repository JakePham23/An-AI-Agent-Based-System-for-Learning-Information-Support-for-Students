...
# Global cache for OpenAI clients to prevent connection exhaustion
...
async def openai_complete_if_cache(
    ...
    **kwargs: Any,
    ...
    # We include all parameters that affect client creation
    client_cache_key = (
        api_key,
        base_url,
        use_azure,
        azure_deployment,
        api_version,
        # Timeout is part of client state, but can be overridden per request
        # However, create_openai_async_client sets it on the client
        # For safety, include it, but typically it assumes one timeout per server config
        timeout,
        # Convert dict to tuple of items for hashing
        tuple(sorted(client_configs.items())),
    )

    should_close_client = False
    
    if client_cache_key in _CLIENT_CACHE:
        openai_async_client = _CLIENT_CACHE[client_cache_key]
        ...
        _CLIENT_CACHE[client_cache_key] = openai_async_client
        logger.debug(f"Created and cached new OpenAI client for {base_url}")
        # should_close_client = True # WE KEEP IT OPEN NOW
