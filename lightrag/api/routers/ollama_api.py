...
def parse_query_mode(query: str) -> tuple[str, SearchMode, bool, Optional[str]]:
    ...
    mode_map = {
        ...
        "/langgraph ": (SearchMode.langgraph, False),
    }
...
class OllamaAPI:
    ...
    def _build_langgraph_app(self):
        from lightrag.api.routers.langgraph_agent import create_agent_app
        return create_agent_app(self.rag)
        ...
        async def chat(raw_request: Request):
            ...
            try:
                ...
                if request.stream:
                    ...
                    elif mode == SearchMode.langgraph:
                        if self.langgraph_app is None:
                            self.langgraph_app = self._build_langgraph_app()
                        inputs = {"messages": [HumanMessage(content=cleaned_query)]}
                        final_state = await self.langgraph_app.ainvoke(inputs)
