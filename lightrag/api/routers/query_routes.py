...
def _get_langgraph_app(rag_instance):
    global langgraph_app
    if langgraph_app is not None:
        return langgraph_app
        
    from lightrag.api.routers.langgraph_agent import create_agent_app
    # Use the shared create_agent_app function which encapsulates all logic
    langgraph_app = create_agent_app(rag_instance)
    return langgraph_app
...
class QueryRequest(BaseModel):
    ...
    mode: Literal["local", "global", "hybrid", "naive", "mix", "bypass", "langgraph"] = Field(
        default="mix",
    ...
    async def query_text_stream(request: QueryRequest):
        ...
        **Error Handling:**
        ...
        try:
            ...
            if request.mode == "langgraph":
                # For streaming endpoint, we just return the full response for now
                app = _get_langgraph_app(rag)
                inputs = {"messages": [HumanMessage(content=request.query)]}
                final_state = await app.ainvoke(inputs)
                response_content = final_state["messages"][-1].content
